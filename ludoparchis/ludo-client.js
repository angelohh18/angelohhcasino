...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePlayerInfoBox(slotColor, player) {
        ...
        if (player) {
            ...
            infoBox.querySelector('.player-name').textContent = player.playerName;
            const avatar = infoBox.querySelector('.player-avatar');
            avatar.src = player.avatar || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs='; // Usa avatar o imagen transparente
        } else {
            infoBox.style.visibility = 'hidden';
            infoBox.querySelector('.player-name').textContent = `Jugador ${slotColor}`;
            infoBox.querySelector('.player-avatar').src = 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=';
        }
    }
    ...
    if (btnBackToLobby && confirmLeaveModal && btnConfirmLeaveYes && btnConfirmLeaveNo) {
        ...
        btnConfirmLeaveYes.addEventListener('click', () => {
            ...
            window.location.href = '/'; // Redirige a la raíz (lobby)
        });
    }
