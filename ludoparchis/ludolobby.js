...
function showGameView(settings) {
    ...
    window.location.href = `/ludo.html?roomId=${settings.roomId}`;
}
...
(function(){
    ...
    btnLogout.addEventListener('click', () => {
        ...
        // MIGRACIÓN SEGURA: Limpiar tanto variables globales como sessionStorage
        currentUser = {
            username: '',
            userAvatar: '',
            userId: '',
        ...
        // ▼▼▼ REEMPLAZA localStorage por sessionStorage ▼▼▼
        ...
        sessionStorage.removeItem('ludo_password');
        // ▲▲▲ FIN DEL REEMPLAZO ▲▲▲
...
function renderRoomsOverview(rooms = []) {
    ...
    function renderLobbyChat(messages = []) {
        ...
        if (messages.length === 0) {
            ...
            welcomeDiv.textContent = '¡Bienvenido al lobby de LUDO! Sé respetuoso y disfruta del juego.';
...
function showRoomsOverview() {
    ...
    function doLogin() {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Guardar en sessionStorage para "recordar" el auto-login
                sessionStorage.setItem('ludo_username', user.name);
                sessionStorage.setItem('ludo_password', password); // ¡Solo para dev local!
                // ▲▲▲ FIN DEL REEMPLAZO ▲▲▲
    ...
    (function init() {
        ...
        // ▲▲▲ FIN DEL BLOQUE AÑADIDO ▲▲▲
         ...
         // ▼▼▼ BLOQUE DE REANUDACIÓN DE SESIÓN MODIFICADO ▼▼▼
 
         // ▼▼▼ REEMPLAZA localStorage por sessionStorage ▼▼▼
         // 1. Buscar una sesión activa en sessionStorage
         const sessionUsername = sessionStorage.getItem('username');
         const sessionCurrency = sessionStorage.getItem('userCurrency');

         if (sessionUsername && sessionCurrency) {
             // 2. El usuario tiene una sesión guardada, reconectar directamente.
             console.log('Reconectando sesión activa para:', sessionUsername);
             
             // 3. Conectar el socket
             socket.connect(); 
             
             // 4. Informar al servidor (el servidor responderá con 'userStateUpdated')
             socket.emit('userLoggedIn', { username: sessionUsername, currency: sessionCurrency });

             // 5. Actualizar la UI localmente (esto es lo que hace 'doLogin' al final)
         ...
         // ▲▲▲ FIN DEL REEMPLAZO ▲▲▲
             ...
             currentUser = {
                 ...
                 // Los créditos se actualizarán automáticamente via 'userStateUpdated'
             };

             // Actualizar la UI del lobby
             document.getElementById('user-name').textContent = sessionUsername;
             ...
             // 6. No hay sesión guardada, mostramos el login modal
             console.log('No se encontró sesión, mostrando modal de login.');
             body.classList.remove('is-logged-in');
             lobbyOverlay.style.display = 'none';
             showLoginModal();
         }
