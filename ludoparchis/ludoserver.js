// server.js (Ludo Game - In-Memory Version) - v3.0
...
console.log('🎮 Servidor Ludo iniciado en modo IN-MEMORY (sin base de datos)');
console.log('⚠️  ADVERTENCIA: Todos los datos se perderán al reiniciar el servidor');
...
function checkAndCleanRoom(roomId, io) {
    ...
    if (!room) {
        ...
        broadcastRoomListUpdate(io); // Notificar por si acaso
        return;
    }
    ...
    if (playersInSeats === 0 && pendingReconnections === 0 && !hasSockets) {
        ...
        // Si la sala está vacía pero tiene reconexiones pendientes, activar limpieza periódica
        if (playersInSeats === 0 && (pendingReconnections > 0 || hasSockets)) {
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        // Guardamos en memoria (contraseña en texto plano para desarrollo local)
        users[username] = {
            username: username,
            password_hash: password, // SIMPLIFICADO: texto plano
            credits: 0.00,
            currency: currency,
            avatar_url: avatar,
            country: country,
        ...
        console.log('✅ Nuevo usuario registrado en memoria:', username);
        res.status(201).json({ success: true, message: 'Usuario registrado exitosamente.' });

    } catch (error) {
        console.error('❌ Error en el registro en memoria:', error);
        res.status(500).json({ 
            success: false, 
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        console.error('❌ Error en el login en memoria:', error);
...
// RUTA DE TEST (IN-MEMORY)
app.get('/api/test-memory', async (req, res) => {
    res.json({
      status: 'success',
    message: 'Servidor funcionando en modo IN-MEMORY',
    data: {
...
// RUTA PARA EL JUEGO LUDO
app.get('/ludo.html', (req, res) => {
...
// Ruta "catch-all" 
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'ludoindex.html'));
});
  ...
  socket.emit('updateRoomList', Object.values(rooms));
    ...
    socket.on('admin:deleteUser', async ({ userId }) => {
        ...
        if (users[username]) {
            ...
            if (userSocket) {
                io.to(userSocket).emit('accountDeleted');
    ...
    socket.on('admin:resetCommissions', () => {
        ...
        io.to('admin-room').emit('admin:commissionsReset');
    });
    ...
    socket.on('admin:updateRates', (newRates) => {
        ...
        io.to('admin-room').emit('admin:ratesUpdated', exchangeRates);
    });
...
// ▲▲▲ FIN DEL BLOQUE REEMPLAZADO ▲▲▲
  ...
  socket.on('disconnect', () => {
    ...
    if (socket.currentRoomId) {
        handlePlayerDeparture(socket.currentRoomId, socket.id, io);
    }
  });
...
/**
 * Verifica si hay salas vacías esperando limpieza (sin jugadores pero con reconexiones pendientes)
 * @param {Server} io - La instancia de Socket.IO
 * @returns {boolean} - True si hay salas vacías esperando limpieza
 */
function hasEmptyRooms(io) {
...
function startPeriodicCleanup(io) {
    if (periodicCleanupInterval !== null) {
        return; // Ya está activo
    }
    
    periodicCleanupInterval = setInterval(() => {
        periodicOrphanRoomCleanup(io);
    }, ORPHAN_ROOM_CLEANUP_INTERVAL_MS);
    
    console.log(`[Periodic Cleanup] Activando limpieza periódica (hay salas vacías esperando limpieza)`);
}
...
function stopPeriodicCleanup() {
    if (periodicCleanupInterval !== null) {
        ...
        console.log(`[Periodic Cleanup] Deteniendo limpieza periódica (no hay salas vacías)`);
    }
}

// Función para limpieza periódica de salas huérfanas
function periodicOrphanRoomCleanup(io) {
    console.log(`[Periodic Cleanup] Iniciando limpieza de salas huérfanas...`);
    const roomIds = Object.keys(rooms);
    let cleanedCount = 0;
    ...
    roomIds.forEach(roomId => {
        ...
        if (playersInSeats === 0 && pendingReconnections === 0 && !hasSockets) {
            console.log(`[Periodic Cleanup] Eliminando sala huérfana: ${roomId}`);
            delete rooms[roomId];
    ...
    if (cleanedCount > 0) {
        console.log(`[Periodic Cleanup] Se eliminaron ${cleanedCount} sala(s) huérfana(s).`);
        broadcastRoomListUpdate(io);
    } else {
        console.log(`[Periodic Cleanup] No se encontraron salas huérfanas.`);
    }
...
// ▼▼▼ MANEJADOR DE ERRORES GLOBAL (EVITA QUE EL SERVIDOR SE APAGUE) ▼▼▼
...
// Iniciar el servidor
server.listen(PORT, () => {
  console.log(`🚀 Servidor Ludo escuchando en http://localhost:${PORT}`);
  console.log(`📁 Modo: IN-MEMORY (sin base de datos)`);
  console.log(`👥 Usuarios en memoria: ${Object.keys(users).length}`);
  console.log(`🎮 Salas activas: ${Object.keys(rooms).length}`);
  
  // La limpieza periódica se activará automáticamente cuando haya mesas vacías
  console.log(`🧹 Limpieza periódica de salas huérfanas se activará automáticamente cuando haya mesas vacías`);
});