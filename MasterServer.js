                ...
                for (const [roomIdKey, ludoRoom] of Object.entries(ludoRooms)) {
                    if (ludoRoom && (ludoRoom.state === 'playing' || ludoRoom.state === 'post-game' || ludoRoom.state === 'waiting')) {
                        ...
                        if (seatIndex !== -1) {
                            ...
                            // ▼▼▼ CRÍTICO: Verificar que el asiento NO esté liberado (null) y que el socket esté conectado Y en la sala ▼▼▼
                            if (seat && seat !== null && seat !== undefined) {
                                ...
                                const roomSockets = io.sockets.adapter.rooms.get(roomIdKey);
                                const isSocketInRoom = roomSockets && roomSockets.has(seatSocket ? seatSocket.id : '');
                                
                                // Solo mantener si el asiento tiene datos, el socket está conectado, el userId coincide Y el socket está en la sala
                                if (isSocketConnected && userIdMatches && isSocketInRoom && (seat.status === 'waiting' || seat.status === 'playing' || ludoRoom.state === 'post-game')) {
                                    shouldKeep = true;
                                    break;
                                } else {
                                    // Asiento fantasma - el jugador ya no está en la sala
                                    console.log(`[broadcastUserListUpdate] 🧹 Asiento fantasma detectado en sala ${roomIdKey}: jugador ${seat.playerName} (socket conectado: ${isSocketConnected}, userId coincide: ${userIdMatches}, socket en sala: ${isSocketInRoom})`);
                                }
                            }
                        }
                    }
...
async function ludoHandlePlayerDeparture(roomId, leavingPlayerId, io, isVoluntaryAbandonment = false, isInactivityTimeout = false) {
    ...
    if (room.state === 'playing' && leavingPlayerSeat.status !== 'waiting') {
        ...
        // ▼▼▼ CRÍTICO: Desconectar el socket de la sala PRIMERO, antes de actualizar el estado ▼▼▼
        ...
        // ▼▼▼ CRÍTICO: Actualizar estado del jugador a "En el lobby" DESPUÉS de desconectar el socket ▼▼▼
        // Esto asegura que broadcastUserListUpdate no vea al jugador como "En mesa de Ludo"
        ...
        const roomSocketsForCleanup = io.sockets.adapter.rooms.get(roomId);
        room.seats.forEach((s, idx) => {
            if (s !== null && s !== undefined && s !== '') {
                const socket = io.sockets.sockets.get(s.playerId);
                if (!socket || !socket.connected) {
                    ...
                    if (socketUserId !== s.userId) {
                        ...
                        // Verificar que el socket esté realmente en la sala de Socket.IO
                        const isSocketInRoom = roomSocketsForCleanup && roomSocketsForCleanup.has(socket.id);
                        if (!isSocketInRoom) {
                            console.log(`[${roomId}] 🧹 Limpiando asiento ${idx} después de desconexión: socket ${s.playerName} no está en la sala de Socket.IO`);
                            room.seats[idx] = null;
                        }
                    }
                }
...
function ludoPassTurn(room, io, isPunishmentTurn = false) {
    ...
    // Cancelar timeout anterior si existe (usar userId para consistencia)
    if (currentSeat && currentSeat.userId) {
        const previousTimeoutKey = `${roomId}_${currentSeat.userId}`;
        if (ludoInactivityTimeouts[previousTimeoutKey]) {
            ...
            console.log(`[${roomId}] Timeout de inactividad cancelado para el jugador anterior (asiento ${currentTurnIndex}, userId: ${currentSeat.userId})`);
        }
        // También cancelar por playerId por si acaso
        const previousTimeoutKeyByPlayerId = `${roomId}_${currentSeat.playerId}`;
        if (ludoInactivityTimeouts[previousTimeoutKeyByPlayerId]) {
            clearTimeout(ludoInactivityTimeouts[previousTimeoutKeyByPlayerId]);
            delete ludoInactivityTimeouts[previousTimeoutKeyByPlayerId];
        }
    }
        ...
        if (mySeatIndex !== -1) {
            ...
            if (room.gameState && room.gameState.turn && room.gameState.turn.playerIndex === mySeatIndex) {
                ...
                if (room.currentPlayerId === oldPlayerId || (room.gameState.turn.playerId === oldPlayerId)) {
                    ...
                    console.log(`[${roomId}] ✅ Actualizado currentPlayerId y turn.playerId de ${oldPlayerId} a ${socket.id} (jugador reconectado en su turno - ludoHandleParchisRoll)`);
                }
            }
            ...
            if (userId) {
                ...
                if (ludoInactivityTimeouts[inactivityTimeoutKey]) {
                    clearTimeout(ludoInactivityTimeouts[inactivityTimeoutKey]);
                    delete ludoInactivityTimeouts[inactivityTimeoutKey];
                    console.log(`[${roomId}] ✓ Timeout de inactividad cancelado para ${userId} (jugador reconectado y actuando - ludoHandleParchisRoll)`);
                }
                ...
                if (ludoDisconnectedPlayers[disconnectKey]) {
                    ...
                    console.log(`[${roomId}] ✓ Estado de desconexión limpiado para ${userId} (jugador reconectado y actuando - ludoHandleParchisRoll)`);
                }
            }
...
async function endGameAndCalculateScores(room, winnerSeat, io, abandonmentInfo = null) {
    ...
    if (room.initialSeats) {
        for (const seat of room.initialSeats) {
            ...
            // ▼▼▼ CRÍTICO: Solo cobrar multa si el jugador fue ELIMINADO por falta, NO si solo no completó el primer meld ▼▼▼
            // La multa SOLO se cobra si:
            // 1. El jugador fue eliminado por una falta (active === false) Y
            // 2. No hay un registro previo en penaltiesPaid (para evitar cobrar dos veces)
            // NO se cobra multa si:
            // - El jugador sigue activo (active === true) aunque no haya completado el primer meld
            // - El jugador completó el primer meld (doneFirstMeld === true) aunque haya perdido
            
            if (wasEliminatedByFault && !hasExistingPenalty && finalSeatState) {
                // El jugador fue eliminado por falta y aún no se cobró la multa - cobrarla ahora
                ...
                if (penalty > 0 && playerInfo) {
                    ...
                    // ▼▼▼ IMPORTANTE: Guardar la multa en penaltiesPaid SOLO cuando realmente se cobra ▼▼▼
                ...
                // Ya hay un registro de penalización - la multa ya fue cobrada anteriormente (por handlePlayerElimination)
                console.log(`[${room.roomId}] Jugador ${seat.playerName} ya tiene multa registrada de falta anterior, no se cobra de nuevo.`);
            } else if (finalSeatState) {
                // El jugador sigue activo (no fue eliminado) - NO se cobra multa, incluso si no completó el primer meld
                const completedFirstMeld = finalSeatState.doneFirstMeld === true;
                console.log(`[${room.roomId}] ✅ Jugador ${finalSeatState.playerName || seat.playerName} NO cometió falta (activo: ${finalSeatState.active}, primer meld: ${completedFirstMeld}), NO se cobra multa.`);
            }
            // ▲▲▲ FIN: SOLO COBRAR MULTA SI HUBO FALTA REAL (ELIMINACIÓN) ▲▲▲
...
async function advanceTurnAfterAction(room, discardingPlayerId, discardedCard, io) {
    ...
    if (previousSeat && previousSeat.userId) {
        const previousTimeoutKey = `${room.roomId}_${previousSeat.userId}`;
        ...
        // También cancelar por playerId por si acaso
        const previousTimeoutKeyByPlayerId = `${room.roomId}_${previousSeat.playerId}`;
        if (la51InactivityTimeouts[previousTimeoutKeyByPlayerId]) {
            clearTimeout(la51InactivityTimeouts[previousTimeoutKeyByPlayerId]);
            delete la51InactivityTimeouts[previousTimeoutKeyByPlayerId];
        }
    }
    ...
    if (nextPlayerSeat && nextPlayerSeat.isBot) {
        ...
        // Solo iniciar timeout si el jugador NO fue eliminado
        if (!alreadyPenalized) {
            // ▼▼▼ CRÍTICO: Verificar si el jugador está desconectado - si lo está, el timeout ya debería estar activo desde disconnect ▼▼▼
            // Si el jugador está desconectado, el timeout ya debería estar activo desde cuando se desconectó
            // Solo iniciar timeout si no está activo (puede que el jugador se haya desconectado y reconectado)
            const disconnectKey = `${room.roomId}_${nextPlayer.userId}`;
            const isDisconnected = la51DisconnectedPlayers[disconnectKey];
            
            // Iniciar timeout solo si no está activo
            if (!la51InactivityTimeouts[newTimeoutKey]) {
                la51InactivityTimeouts[newTimeoutKey] = setTimeout(() => {
                    ...
                    // Verificar por userId (el playerId puede haber cambiado si se reconectó)
                    const currentSeat = currentRoom.seats.find(s => s && s.userId === nextPlayer.userId);
                    if (!currentSeat) {
                    ...
                    if (currentSeat.userId !== nextPlayer.userId) {
                        console.log(`[${room.roomId}] El jugador en el asiento ya no es el mismo. No se elimina por inactividad.`);
                        delete la51InactivityTimeouts[newTimeoutKey];
                        return;
                    }
                    
                    // Verificar que el turno todavía es de este jugador (por userId)
                    if (currentRoom.currentPlayerId !== currentSeat.playerId) {
                        // Verificar si el currentPlayerId apunta a otro asiento del mismo userId
                        const currentPlayerSeat = currentRoom.seats.find(s => s && s.playerId === currentRoom.currentPlayerId);
                        if (!currentPlayerSeat || currentPlayerSeat.userId !== nextPlayer.userId) {
                    ...
                    // ▼▼▼ CRÍTICO: REGISTRAR EN penaltyApplied Y la51EliminatedPlayers ANTES DE ELIMINAR ▼▼▼
                    if (!currentRoom.penaltyApplied) {
                        currentRoom.penaltyApplied = {};
                    }
                    currentRoom.penaltyApplied[currentSeat.userId] = true;
                    
                    // Registrar en la51EliminatedPlayers para que no pueda reconectarse después
                    const eliminatedKey = `${room.roomId}_${currentSeat.userId}`;
                    la51EliminatedPlayers[eliminatedKey] = {
                        playerName: currentSeat.playerName,
                        reason: 'Abandono por inactividad',
                        faultData: { reason: 'Abandono por inactividad' },
                        penaltyInfo: {
                            amount: parseFloat(room.settings.penalty) || 0,
                            reason: 'Abandono por inactividad'
                        }
                    };
                    console.log(`[${room.roomId}] ✅ Jugador ${currentSeat.playerName} registrado en penaltyApplied y la51EliminatedPlayers.`);
                    ...
                    if (playerIdToUse) {
                        handlePlayerDeparture(room.roomId, playerIdToUse, io, false, true); // false = no es abandono voluntario, true = es timeout de inactividad
                    } else {
                        // Si no hay playerId, eliminar directamente el asiento y pasar el turno
                        console.log(`[${room.roomId}] ⚠️ Jugador ${currentSeat.playerName} está desconectado sin socket. Eliminando asiento directamente.`);
                        const seatIndexToEliminate = currentRoom.seats.findIndex(s => s && s.userId === currentSeat.userId);
                        if (seatIndexToEliminate !== -1) {
                            currentRoom.seats[seatIndexToEliminate] = null;
                            
                            // Pasar el turno si era su turno
                            if (currentRoom.currentPlayerId === currentSeat.playerId || (currentSeat.userId && currentRoom.seats.find(s => s && s.playerId === currentRoom.currentPlayerId)?.userId === currentSeat.userId)) {
                                ...
                                if (activeSeats.length > 1) {
                                    ...
                                    while (attempts < currentRoom.seats.length * 2) {
                                        ...
                                        if (nextSeat && nextSeat.active !== false) {
                                            ...
                                            // Iniciar timeout para el nuevo jugador usando advanceTurnAfterAction
                                            const nextPlayerSeat = currentRoom.seats.find(s => s && s.playerId === nextSeat.playerId);
                                            if (nextPlayerSeat && !nextPlayerSeat.isBot) {
                                                const nextTimeoutKey = `${room.roomId}_${nextPlayerSeat.userId}`;
                                                if (!la51InactivityTimeouts[nextTimeoutKey]) {
                                                    // Usar la misma lógica que en advanceTurnAfterAction
                                                    const alreadyPenalizedNext = (currentRoom.penaltyApplied && currentRoom.penaltyApplied[nextPlayerSeat.userId]);
                                                    if (!alreadyPenalizedNext) {
                                                        la51InactivityTimeouts[nextTimeoutKey] = setTimeout(() => {
                                                            // Lógica de timeout (similar a la de arriba)
                                                            handlePlayerDeparture(room.roomId, nextSeat.playerId, io, false, true);
                                                            delete la51InactivityTimeouts[nextTimeoutKey];
                                                        }, LA51_INACTIVITY_TIMEOUT_MS);
                                                    }
                                                }
                                            } else if (nextPlayerSeat && nextPlayerSeat.isBot) {
                                                setTimeout(() => botPlay(currentRoom, nextSeat.playerId, io), 1000);
                                            }
                                            break;
                                        }
                ...
                console.log(`[${room.roomId}] ⚠️ Timeout de inactividad ya está activo para ${nextPlayer.playerName} (userId: ${nextPlayer.userId}). No se reinicia.`);
            }
        }
  ...
  socket.on('requestPracticeGame', ({ username, avatar }) => { // <--- Recibimos un objeto
    ...
    socket.on('joinRoom', async ({ roomId, user }) => {
        ...
        if (existingSeat && existingSeatIndex !== -1) {
            ...
            console.log(`[${roomId}] [RECONEXIÓN] Iniciando búsqueda de mano para ${user.username} (${userId})...`);
            console.log(`[${roomId}] [RECONEXIÓN] oldPlayerId: ${oldPlayerId}, nuevo socket.id: ${socket.id}`);
            console.log(`[${roomId}] [RECONEXIÓN] room.playerHands existe?: ${!!room.playerHands}`);
            if (room.playerHands) {
                console.log(`[${roomId}] [RECONEXIÓN] Claves en playerHands:`, Object.keys(room.playerHands));
                console.log(`[${roomId}] [RECONEXIÓN] playerHands[${oldPlayerId}] existe?: ${!!room.playerHands[oldPlayerId]}`);
                if (room.playerHands[oldPlayerId]) {
                    console.log(`[${roomId}] [RECONEXIÓN] playerHands[${oldPlayerId}] tiene ${room.playerHands[oldPlayerId].length} cartas`);
                }
            }
            ...
            if (room.playerHands && room.playerHands[oldPlayerId] && Array.isArray(room.playerHands[oldPlayerId]) && room.playerHands[oldPlayerId].length > 0) {
                playerHandToRestore = [...room.playerHands[oldPlayerId]]; // Crear copia para evitar referencias
                ...
                console.log(`[${roomId}] ⚠️ [RECONEXIÓN] No se encontró mano con oldPlayerId ${oldPlayerId}. Buscando en todos los playerHands por userId ${userId}...`);
                if (room.playerHands) {
                    ...
                    for (const [playerIdKey, hand] of Object.entries(room.playerHands)) {
                        if (Array.isArray(hand) && hand.length > 0) {
                            ...
                            if (seatWithThisPlayerId) {
                                playerHandToRestore = [...hand]; // Crear copia para evitar referencias
            ...
            if (playerHandToRestore && playerHandToRestore.length > 0) {
                // Asegurar que playerHands existe
                if (!room.playerHands) {
                    room.playerHands = {};
                }
                ...
                // Asignar la mano al nuevo socket.id
                ...
                console.log(`[${roomId}] [RECONEXIÓN] Verificación: playerHands[${socket.id}] ahora tiene ${room.playerHands[socket.id].length} cartas`);
            } else {
                console.error(`[${roomId}] ❌ [RECONEXIÓN] ERROR: No se pudo encontrar la mano del jugador ${user.username} (${userId})`);
                console.error(`[${roomId}] [RECONEXIÓN] playerHands completo:`, room.playerHands);
            }
            ...
            if (room.state === 'playing') {
                ...
                if (!room.playerHands || !room.playerHands[socket.id] || room.playerHands[socket.id].length === 0) {
                    console.error(`[${roomId}] ❌ [RECONEXIÓN] ERROR: playerHands[${socket.id}] está vacío o no existe después de actualización inicial. Buscando en todos los playerHands...`);
                    // Buscar en TODOS los playerHands directamente por userId, sin depender de seat.playerId
                    let foundHand = null;
                    for (const [playerIdKey, hand] of Object.entries(room.playerHands || {})) {
                        if (Array.isArray(hand) && hand.length > 0) {
                            // Verificar si este playerId corresponde a un asiento con el mismo userId
                            ...
                            if (seatWithThisPlayerId) {
                                foundHand = hand;
                                room.playerHands[socket.id] = foundHand;
                                console.log(`[${roomId}] ✅ [RECONEXIÓN] Mano encontrada en playerHands[${playerIdKey}] y actualizada a ${socket.id}. Mano tiene ${foundHand.length} cartas`);
                                break;
                            }
                        }
                    }
                    if (!foundHand || foundHand.length === 0) {
                        console.error(`[${roomId}] ❌ [RECONEXIÓN] ERROR CRÍTICO: No se pudo encontrar la mano del jugador ${user.username} (${userId}) en ningún playerHands`);
                    }
                } else {
...
socket.on('accionDescartar', async (data) => {
  ...
  socket.on('disconnect', () => {
    ...
    if (roomId && userId) {
        if (ludoRooms[roomId]) {
            ...
            hasActiveInactivityTimeout = ludoInactivityTimeouts[ludoTimeoutKey];
        }
        if (la51Rooms[roomId]) {
            const la51TimeoutKey = `${roomId}_${userId}`;
            hasActiveInactivityTimeout = hasActiveInactivityTimeout || la51InactivityTimeouts[la51TimeoutKey];
        }
    }
    ...
    if (roomId && ludoRooms[roomId]) {
        ...
        if (seatIndex !== -1 && room.state === 'waiting') {
            ...
            // ▼▼▼ CRÍTICO: El jugador se desconectó durante una partida activa - NO hacer nada, el timeout se encargará ▼▼▼
            const leavingPlayerSeat = room.seats[seatIndex];
            ...
            // ▲▲▲ FIN: NO HACER NADA EN PARTIDA ACTIVA - EL TIMEOUT SE ENCARGARÁ ▲▲▲
        ...
        // ESTABA EN UNA SALA DE LA 51 - LÓGICA SIMPLIFICADA IGUAL QUE LUDO
        ...
        if (seatIndex !== -1 && la51Room.state === 'waiting') {
            // ESTABA EN EL LOBBY DE LA 51 (ESPERANDO) - Eliminar inmediatamente
  ...
  // ▼▼▼ HANDLERS DE SOCKET DE LUDO (copiados de ludoserver.js, cambiando rooms por ludoRooms) ▼▼▼
    ...
    socket.on('joinLudoGame', (data) => {
      ...
      if (mySeatIndex !== -1) {
          const existingSeat = room.seats[mySeatIndex];
          // ▼▼▼ CRÍTICO: Guardar el playerId antiguo ANTES de actualizarlo ▼▼▼
          const oldPlayerId = existingSeat.playerId;
          console.log(`[${roomId}] 🔄 Jugador ${userId} reconectándose a su asiento [${mySeatIndex}]. Actualizando socket.id de ${oldPlayerId} a ${socket.id}`);
          ...
          // ▼▼▼ CRÍTICO: Cancelar TODOS los timeouts de inactividad usando oldPlayerId, socket.id y userId ▼▼▼
          const inactivityTimeoutKey = `${roomId}_${userId}`;
          if (ludoInactivityTimeouts[inactivityTimeoutKey]) {
              clearTimeout(ludoInactivityTimeouts[inactivityTimeoutKey]);
              delete ludoInactivityTimeouts[inactivityTimeoutKey];
          ...
          // Cancelar timeout usando el playerId antiguo
          if (oldPlayerId && oldPlayerId !== socket.id) {
              const oldTimeoutKey = `${roomId}_${oldPlayerId}`;
              if (ludoInactivityTimeouts[oldTimeoutKey]) {
                  clearTimeout(ludoInactivityTimeouts[oldTimeoutKey]);
          ...
          // Cancelar timeout usando el nuevo socket.id (por si acaso)
          const newTimeoutKey = `${roomId}_${socket.id}`;
          if (ludoInactivityTimeouts[newTimeoutKey]) {
              clearTimeout(ludoInactivityTimeouts[newTimeoutKey]);
              delete ludoInactivityTimeouts[newTimeoutKey];
              console.log(`[${roomId}] ✓ Timeout cancelado usando nuevo socket.id: ${socket.id}`);
          ...
          Object.keys(ludoInactivityTimeouts).forEach(key => {
              if (key.startsWith(`${roomId}_`) && (key.includes(userId) || key.includes(socket.id) || (oldPlayerId && key.includes(oldPlayerId)))) {
                  clearTimeout(ludoInactivityTimeouts[key]);
                  delete ludoInactivityTimeouts[key];
                  console.log(`[${roomId}] ✓ Timeout adicional cancelado: ${key}`);
              }
    ...
    socket.on('ludoRollDice', (data) => {
      ...
      // ▼▼▼ CRÍTICO: Buscar asiento por socket.id primero, luego por userId (para casos de reconexión) ▼▼▼
      const userId = socket.userId || (socket.handshake && socket.handshake.auth && socket.handshake.auth.userId);
      let mySeatIndex = room.seats.findIndex(s => s && s.playerId === socket.id);
      ...
      if (mySeatIndex === -1 && userId) {
          ...
          if (mySeatIndex !== -1) {
              ...
              const oldPlayerId = existingSeat.playerId;
              existingSeat.playerId = socket.id;
              console.log(`[${roomId}] ✅ Actualizado playerId del asiento ${mySeatIndex} de ${oldPlayerId} a ${socket.id} para userId ${userId} (ludoRollDice - reconexión)`);
    ...
    socket.on('ludoMovePiece', async (data) => {
          ...
          if (mySeatIndex !== -1) {
              ...
              if (room.gameState && room.gameState.turn && room.gameState.turn.playerIndex === mySeatIndex) {
                  ...
                  if (room.currentPlayerId === oldPlayerId || (room.gameState.turn.playerId === oldPlayerId)) {
                      ...
                      console.log(`[${roomId}] ✅ Actualizado currentPlayerId y turn.playerId de ${oldPlayerId} a ${socket.id} (jugador reconectado en su turno - ludoMovePiece)`);
                  }
              }
              // ▲▲▲ FIN ACTUALIZACIÓN DE CURRENTPLAYERID ▲▲▲
              ...
              if (userId) {
                  ...
                  if (ludoInactivityTimeouts[inactivityTimeoutKey]) {
                      clearTimeout(ludoInactivityTimeouts[inactivityTimeoutKey]);
                      delete ludoInactivityTimeouts[inactivityTimeoutKey];
                      console.log(`[${roomId}] ✓ Timeout de inactividad cancelado para ${userId} (jugador reconectado y actuando - ludoMovePiece)`);
                  }
                  ...
                  if (ludoDisconnectedPlayers[disconnectKey]) {
                      delete ludoDisconnectedPlayers[disconnectKey];
                      console.log(`[${roomId}] ✓ Estado de desconexión limpiado para ${userId} (jugador reconectado y actuando - ludoMovePiece)`);
                  }
              }
              // ▲▲▲ FIN CANCELACIÓN TIMEOUT Y LIMPIEZA DESPUÉS DE RECONEXIÓN ▲▲▲
          }
      ...
      // CRÍTICO: Solo hacer esto si NO se encontró el asiento por userId (ya se canceló arriba si se encontró)
      if (mySeatIndex !== -1 && userId) {
          const inactivityTimeoutKey = `${roomId}_${userId}`;
          if (ludoInactivityTimeouts[inactivityTimeoutKey]) {
              clearTimeout(ludoInactivityTimeouts[inactivityTimeoutKey]);
              delete ludoInactivityTimeouts[inactivityTimeoutKey];
              console.log(`[${roomId}] ✓ Timeout de inactividad cancelado para ${userId} (jugador movió una ficha)`);
          }
